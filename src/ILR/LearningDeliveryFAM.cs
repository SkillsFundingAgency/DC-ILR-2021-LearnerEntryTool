...
namespace ILR
{
    public class LearningDeliveryFAM : ChildEntity, IDataErrorInfo
    {
        ...
        public enum SingleOccurrenceFAMs
        {
            SOF,
            FFI,
            EEF,
        ...
        {
            get { return XMLHelper.GetChildValue("LearnDelFAMType", Node, NSMgr); }
            set
            {
                XMLHelper.SetChildValue("LearnDelFAMType", value, Node, NSMgr);
                OnPropertyChanged("LearnDelFAMType");
            }
        }

        public string LearnDelFAMCode
        {
            get { return XMLHelper.GetChildValue("LearnDelFAMCode", Node, NSMgr); }
            set
            {
                XMLHelper.SetChildValue("LearnDelFAMCode", value, Node, NSMgr);
                OnPropertyChanged("LearnDelFAMCode");
            }
        }
        ...
        public DateTime? LearnDelFAMDateFrom
        {
            get
            {
                string LearnDelFAMDateFrom = XMLHelper.GetChildValue("LearnDelFAMDateFrom", Node, NSMgr);
                return (LearnDelFAMDateFrom != null ? DateTime.Parse(LearnDelFAMDateFrom) : (DateTime?) null);
            }
            set
            {
                XMLHelper.SetChildValue("LearnDelFAMDateFrom", value, Node, NSMgr);
                OnPropertyChanged("LearnDelFAMDateFrom");
            }
        }

        public DateTime? LearnDelFAMDateTo
        {
            get
            {
                string LearnDelFAMDateTo = XMLHelper.GetChildValue("LearnDelFAMDateTo", Node, NSMgr);
                return (LearnDelFAMDateTo != null ? DateTime.Parse(LearnDelFAMDateTo) : (DateTime?) null);
            }
            set
            {
                XMLHelper.SetChildValue("LearnDelFAMDateTo", value, Node, NSMgr);
        ...
        internal LearningDeliveryFAM(LearningDeliveryFAM MigrationLearningDeliveryFAM, XmlNode LearningDeliveryFAMNode,
            XmlNamespaceManager NSMgr)
        {
            this.Node = LearningDeliveryFAMNode;
            this.NSMgr = NSMgr;

            if ((MigrationLearningDeliveryFAM.LearnDelFAMType == "LDM" &&
                 MigrationLearningDeliveryFAM.LearnDelFAMCode == "125") ||
                MigrationLearningDeliveryFAM.LearnDelFAMType == "WPL")
            {
